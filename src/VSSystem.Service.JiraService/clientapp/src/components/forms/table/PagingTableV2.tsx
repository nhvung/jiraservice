...
import { getElementById, isInElement } from '../../../extensions/ElementExtension';
import { guid8 } from '../../../extensions/HashFuncs';
import { componentProps } from '../../componentDefine';
import { cellProps, dataResponse, pagingTableProps, rowProps } from './PagingTable';
import thisCss from './PagingTable.module.scss';

function PagingTableV2(props: componentProps & pagingTableProps) {
    ...
    try {
        ...
        if (props.onInitMessage) {
            bodyElement = (<tr><td align='center' colSpan={props.headers?.length + (props.includeCheckbox ? 1 : 0)}>{props.onInitMessage}</td></tr>);
        }
        if (!loaded) {
            if (props.onLoadingMessage) {
                bodyElement = (<tr><td align='center' colSpan={props.headers?.length + (props.includeCheckbox ? 1 : 0)}>{props.onLoadingMessage}</td></tr>);
            }
        }
        else {
            bodyElement = (<tr><td align='center' colSpan={props.headers?.length + (props.includeCheckbox ? 1 : 0)}>No Record.</td></tr>);
        ...
        let contents = (<div {...props} id={thisId} className={className}
            ...
            <div className='pTable' style={{ height: `calc(100% - ${pagingHeight}px)` }} >
                ...
                >
                    {headerElement}
                    <tbody id={bodyId} className={bodyClassName}>
                        {bodyElement}
                    </tbody>
                </table>
            </div>
